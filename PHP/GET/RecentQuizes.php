...
<?php
    ...
    if($_GET['self']==0)
        ...
        {
            $q = "SELECT * FROM quizes AS t1 
                    JOIN quizattributes t2 ON t1.id = t2.quizid WHERE quizid IN (select favorites.quizid 
            from favorites
            where userid=?)
                    ...
                    foreach ($results as $quiz)
                        {
                            $isFavorite = isFavorite($user->ID, $quiz['id']);
                             ...
                             echo'<td>'.Editable($quiz['idcreator'],$user->ID,$quiz['id']).'</td>';//No Of questions
                             echo '<td>'.setDeletionIcon($quiz['idcreator'],$user->ID,$quiz['id']).'</td>';
                             echo "</tr>";
                         }
                    echo    '</table>';
                }
                    ...
                    foreach ($results as $quiz)
                        {
                            ...
                            echo    "<td>".$quiz['sdesc']."</td>";
                            echo    '<td>'.countQuestions($quiz['id']).'</td>';//No Of questions
                            echo    '<td>'.createdBy($quiz['id'],-1).'</td>';
//                                if($quiz['viewable']==1)
//                                    {
//                                        echo'<td>Yes</td>';
//                                    }
